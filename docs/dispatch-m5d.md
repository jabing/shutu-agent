...
> 状态：待 M5a/M5b/M5c 验收后派发 2026-08-18（M5 拆四段：M5a 后台任务 → M5b 子代理 → M5c 上下文压缩 → M5d 技能；本文件为第四段）· 用法：M5c 验收通过后，把下文整段粘贴给新开的实施会话。
...
1. **`internal/skill` 包——技能注册表（Service 定义，多 Provider）+ 文件系统 Provider（默认）+ 目录/加载工具（Consumer）**：
   ...
   - **目录注入（D3）**：会话开始时（pre-step 注入器，M5b 统一机制）把技能**目录**（排序后的 `name + description`，`catalog_max_chars` 有界，不塞正文/路径/来源）注入为上下文消息，并落 `skill/catalog` 事件。目录变更由组合根按需重读（下一次 pre-step 重取；不引入文件监视）。
...
- **技能正文安全**：技能是本地可信文件，加载后作为模型指令输入，**不执行**；`skill_load` 返回正文有长度上限（`catalog_max_chars` 之外再设 `body_max_chars`，默认 8000，超出截断）。
...
**自测（全部通过后提交，提交信息含 M5d）**：`go vet ./...`、`go test ./...`、`go build ./...`。新增测试至少覆盖：文件系统发现（目录束 + 平铺 + 非递归 + frontmatter 解析）、同名 rank 裁决、`Get` 加载完整正文 + name 失配拒绝、目录注入有界（`catalog_max_chars` 截断）+ `skill/catalog` 事件、`skill_load` 工具（kebab-case 校验、正文长度上限、`tool/result`）、`skill/*` 事件类型可落日志、skill 默认关闭（enabled=false 不初始化）。
