...
你的任务是实现 Go 项目 `D:\dev-projects\Agent\shutu-agent` 的 **M6b-2：`plan_*` 工具 + `plan/*` 事件 + config + 组合根接线 + 单元测试**。这是 M6b 的第二半（第一半 M6b-1 已做 `internal/plan` 接缝 + 域模型，你**依赖它们**）。你是实施会话。

**直接开工，不要做任何前置检查**。你的主要输入是 `D:\dev-projects\Agent\shutu-agent\docs\dispatch-m6b-2.md`（本文件即主契约），**先完整读它**，然后立即用 write 工具创建/修改文件。

**读这些（按需精读片段，不要通读）**：
1. `D:\dev-projects\Agent\shutu-agent\docs\dispatch-m6b-1.md` —— 接缝契约（Engine/Goal/Plan/Todo/Status/Provider 签名）。
2. `D:\dev-projects\Agent\shutu-agent\docs\decisions\2026-08-19-m6-agent-full.md` —— M6 主 ADR（M6b 行）。
3. 现有代码（按需精读）：
...
git 提交：`git -C D:\dev-projects\Agent\shutu-agent -c user.name='Personal Agent' -c user.email='dev@shutu-agent.local' commit -m "M6b-2: <what>"`。不要提交 pa.exe/data/缓存。
